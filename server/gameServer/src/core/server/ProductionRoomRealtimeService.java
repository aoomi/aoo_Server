...
final class ProductionRoomRealtimeService {
    ...
    GameCommandResult dispatch(Player player, ProtocolV2AuthorityRuntime.Command command) {
        ...
        try {
            ...
            String socialEvent = socialEvent(action);
            if (socialEvent != null) broadcastSocial(room, seat, player, command, body, socialEvent);
     ...
     */
    private static void broadcastSocial(AbsBaseRoom room, AbsRoomPos seat, Player player,
                                        ProtocolV2AuthorityRuntime.Command command,
                                        Map<String,Object> response, String eventName) {
        Map<String,Object> event = new java.util.LinkedHashMap<>(response);
        event.put("messageId", command.requestId());
        event.put("requestId", command.requestId());
        event.put("serverSeq", command.sequence());
        event.put("seatId", seat.getPosID());
        event.put("sourceSeatId", seat.getPosID());
        event.put("senderPid", player.getPid());
        room.getRoomPosMgr().getPosList().stream().filter(java.util.Objects::nonNull)
                .filter(position -> !position.isRobot() && position.getPid() > 0L)
                .forEach(position -> position.getPlayer().pushProto(eventName, event));
        room.getRoomPosMgr().getWatchList().stream().filter(java.util.Objects::nonNull)
                .filter(position -> !position.isRobot() && position.getPid() > 0L)
                .forEach(position -> position.getPlayer().pushProto(eventName, event));
    }
    private static String socialEvent(String action){return switch(action){case "quick_text" -> "room.quick_text";case "magic_expression" -> "room.magic_expression";case "voice" -> "room.voice";default -> null;};}

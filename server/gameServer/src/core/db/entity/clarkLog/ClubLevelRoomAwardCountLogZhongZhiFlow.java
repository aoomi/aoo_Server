 ...
 */
@TableName(value = "ClubLevelRoomAwardCountLogZhongZhi")
@Data
@NoArgsConstructor
public class ClubLevelRoomAwardCountLogZhongZhiFlow extends BaseClarkLogEntity<ClubLevelRoomAwardCountLogZhongZhiFlow> {
    ...
    public ClubLevelRoomAwardCountLogZhongZhiFlow(String date_time, int setCount, int winner, int consume, long roomSize, long upLevelId, long memberId, long sumPoint, double sportsPoint, double sportsPointConsume, double roomSportsPointConsume, double roomAvgSportsPointConsume, long clubId, long unionId, double promotionShareValue, int sizePlayer, String shareValue, double scorePoint, double personalSportsPoint,int awardNum) {
    ...
    public String getInsertSql() {
        return "INSERT INTO ClubLevelRoomAwardCountLogZhongZhi"
                + "(`server_id`, `timestamp`, `date_time`, `setCount`, `winner`, `consume`, `roomSize`, `upLevelId`, `memberId`, `sumPoint`, `sportsPoint`, `sportsPointConsume`, `roomSportsPointConsume`, `roomAvgSportsPointConsume`, `clubId`,`unionId`,`promotionShareValue`,`sizePlayer`,`shareValue`,`scorePoint`,`personalSportsPoint`,`awardNum`)"
                + "values(?, ?, ?, ?, ?, ?, ?, ? ,? ,? ,? ,? ,? ,?,?,?,?,?,?,?,?,?)";
    }


    public static String getCreateTableSQL() {
        String sql = "CREATE TABLE IF NOT EXISTS `ClubLevelRoomAwardCountLogZhongZhi` ("
                ...
                + "PRIMARY KEY (`id`),"
                + "KEY `unionId_awardNum` (`unionId`,`awardNum`) USING BTREE,"
                + "KEY `date_time, memberId` (`date_time`,`memberId`)"
                + ") COMMENT='玩家推广员及下属消耗中至特殊表(颁奖记录所用)' DEFAULT CHARSET=utf8";
        return sql;
    }

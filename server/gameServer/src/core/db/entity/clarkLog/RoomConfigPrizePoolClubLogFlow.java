 ...
 */
@TableName(value = "roomConfigPrizePoolClubLog")
@Data
@NoArgsConstructor
public class RoomConfigPrizePoolClubLogFlow extends BaseClarkLogEntity<RoomConfigPrizePoolClubLogFlow> {
    ...
    public RoomConfigPrizePoolClubLogFlow(String date_time, long configId, long roomId, int setCount, int roomSize, int value, long clubId, String roomName, String dataJsonCfg, double prizePool, int gameId, double sportPointIncome, double roomSportsPointConsume) {
    ...
    public String getInsertSql() {
        return "INSERT INTO roomConfigPrizePoolClubLog"
                + "(`server_id`, `timestamp`, `date_time`, `configId`, `roomId`, `setCount`, `roomSize`, `consumeValue`, `clubId`, `roomName`, `dataJsonCfg`,`prizePool`,`gameId`,`sportPointIncome`,`roomSportsPointConsume`)"
                + "values(?, ?, ?, ?, ?, ?, ?, ? ,? ,? ,?,?,?,?,?)";
    }

    public static String getCreateTableSQL() {
        String sql = "CREATE TABLE IF NOT EXISTS `roomConfigPrizePoolClubLog` ("
                ...
                + "PRIMARY KEY (`id`),"
                + "KEY `clubId_date_time_roomName` (`clubId`,`date_time`,`roomName`),"
                +" KEY `configId_roomId` (`configId`,`roomId`)"
                + ") COMMENT='奖金池与玩法经营(普通亲友圈记录的信息)' DEFAULT CHARSET=utf8";
        return sql;
    }

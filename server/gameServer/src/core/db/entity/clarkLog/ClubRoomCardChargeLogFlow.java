...
public class ClubRoomCardChargeLogFlow extends BaseClarkLogEntity<ClubRoomCardChargeLogFlow> {
    ...
    public ClubRoomCardChargeLogFlow(long pid, long clubID, int status, int reason, int num, int cur_remainder, int pre_value, int type, int gameType, int rechargeType, long agentsID, int level, int classificationRegionId) {
        this.pid = pid;
        this.status = status;
        this.reason = reason;
        this.num = num;
    ...
    public String getInsertSql() {
        return "INSERT INTO ClubRoomCardChargeLog"
                + "(`server_id`, `timestamp`, `date_time`, `pid`, `clubID`, `status`, `reason`, `num`, `cur_remainder`, `pre_value`, `type`,`gameType`,`rechargeType`,`agentsID`,`level`,`classificationRegionId`)"
                + "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)";
    }

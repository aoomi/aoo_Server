...
public class UnionBO extends BaseEntity<UnionBO> {
    ...
    @DataBaseField(type = "bigint(10)", fieldname = "union_total_score", comment = "联盟总分")
    ...
    private int roundId;
    @DataBaseField(type = "varchar(50)", fieldname = "clubName", comment = "主办俱乐部名称")
    ...
    @DataBaseField(type = "int(2)", fieldname = "joinClubSameUnion", comment = "允许俱乐部添加同赛事玩家 0:允许,1:不允许", defaultValue = "1")
    ...
    @DataBaseField(type = "int(2)", fieldname = "showUplevelId", comment = "显示上级及所属俱乐部")
     ...
     * 联盟总分
     */
    public long getUnionTotalScore() {
        return unionTotalScore;
     ...
     * 联盟总分
     *
     * @param unionTotalScore
     */
    public void setUnionTotalScore(long unionTotalScore) {
     ...
     * 保存初始联盟总分
     *
     * @param unionTotalScore 联盟总分
     */
    public void saveUnionTotalScore(long unionTotalScore) {
        Map<String, Object> update = com.google.common.collect.Maps.newHashMapWithExpectedSize(2);
        this.unionTotalScore = unionTotalScore;
        this.newUnionTime = CommTime.nowSecond();
        update.put("union_total_score", this.unionTotalScore);
        update.put("newUnionTime", this.newUnionTime);
        getBaseService().update(update, id, new AsyncInfo(id));
        if (Config.isShare()) {
            ShareUnionListMgr.getInstance().updateUnionBoField(this, "unionTotalScore", "newUnionTime");
        }
     ...
     */
    public void saveMap(String name, int join, int quit, int tableNum, int expression, int state, int sports, long unionTotalScore, int matchRate, double outSports, int prizeType, int ranking, int value, long exePid, int joinClubSameUnion) {
        HashMap<String, Object> map = new HashMap<>();

        if (unionTotalScore != this.unionTotalScore) {
            long oldUnionTotalScore = this.unionTotalScore;
            // 联盟总分修改
            this.unionTotalScore = unionTotalScore;
            map.put("union_total_score", this.unionTotalScore);
            this.insertUnionDynamicBO(exePid, UnionDefine.UNION_EXEC_TYPE.UNION_EXEC_TOTAL_SCORE, String.valueOf(this.unionTotalScore), String.valueOf(this.unionTotalScore), String.valueOf(oldUnionTotalScore));
        }
    ...
    /**
     * 添加赛事动态记录
     * 联盟总分修改：玩家@玩家名称[ID:@id] 修改了联赛的联盟总分为@值，修改前为@值；
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `bigUnion` ("
                ...
                + "`union_total_score` bigint(10) NOT NULL DEFAULT '0' COMMENT '联盟总分',"
                ...
                + "`clubName` varchar(50) NOT NULL DEFAULT ''  COMMENT '主办俱乐部名称',"
                ...
                + "`joinClubSameUnion` int(2) NOT NULL DEFAULT '1' COMMENT '允许俱乐部添加同赛事玩家 0:允许,1:不允许',"
                ...
                + "`showUplevelId` int(2) NOT NULL DEFAULT '0' COMMENT '显示上级及所属俱乐部)',"

...
public class ClubMemberRelationBO extends BaseEntity<ClubMemberRelationBO> {
    ...
    private long id;
    @DataBaseField(type = "bigint(20)", fieldname = "uid", comment = "俱乐部成员Id")
    private long uid;
    @DataBaseField(type = "bigint(20)", fieldname = "puid", comment = "上级俱乐部成员Id")
    private long puid;
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `ClubMemberRelation` ("
                ...
                + "`uid` bigint(20) NOT NULL DEFAULT '0' COMMENT '俱乐部成员Id',"
                + "`puid` bigint(20) NOT NULL DEFAULT '0' COMMENT '上级俱乐部成员Id',"
                + "PRIMARY KEY (`id`),"
                + "UNIQUE KEY `uid_puid` (`uid`,`puid`) USING BTREE,"
                + "KEY `uid` (`uid`),"
                + "KEY `puid` (`puid`)"
                + ") COMMENT='俱乐部成员级别关联表'  DEFAULT CHARSET=utf8 AUTO_INCREMENT=" + (Constant.InitialID + 1);
        return sql;
    }
}

...
public class UnionDynamicBO extends BaseEntity<UnionDynamicBO> {
    ...
    private long pid;
    @DataBaseField(type = "bigint(20)", fieldname = "clubId", comment = "所属玩家俱乐部Id")
    private long clubId;
    ...
    @DataBaseField(type = "bigint(20)", fieldname = "execClubId", comment = "执行操作玩家俱乐部Id")
    ...
    @DataBaseField(type = "int(11)", fieldname = "type", comment = "类型（0联盟1俱乐部）")
    private int type;
    ...
    /**
     * 	加入赛事：俱乐部@俱乐部名称[ID:@id]被@玩家名称[id：@id]审核加入了赛事；
     * 	关闭审核时：俱乐部@俱乐部名称[ID:@id]加入了赛事
     * 	加入赛事：俱乐部@俱乐部名称[ID:@id]被@玩家名称[id：@id]邀请加入了赛事；
     * 	踢出赛事：俱乐部@俱乐部名称[ID:@id]被@玩家名称[id：@id]踢出了赛事；
     * 	退出赛事：俱乐部@俱乐部名称[ID:@id]退出了赛事；
     * 	需审核时：俱乐部@俱乐部名称[ID:@id]被@玩家名称[id：@id]审核退出了赛事
     * <p>
    ...
    /**
     * 竞技点变动
     * 	俱乐部圈主被赛事管理或盟主增加竞技点：玩家@玩家名称[ID:@id]被@玩家名称[id：@id]增加了竞技点值@值；
     * 	俱乐部圈主被赛事管理或盟主增加/减少竞技点：玩家@玩家名称[ID:@id]被@玩家名称[id：@id]减少了竞技点值@值；
     *
     * @param pid        玩家Pid
     * @param unionId    联赛Id
     * @param clubId     被操作者俱乐部Id
     * @param execPid    操作者Pid
     * @param execClubId 操作者俱乐部Id
     * @param execTime   操作时间
     * @param execType   操作类型
     * @param value      操作值
     * @param type       类型
...
//    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `unionDynamic` ("
                ...
                + "`clubId` bigint(20) NOT NULL DEFAULT '0' COMMENT '所属玩家俱乐部Id',"
                ...
                + "`execClubId` bigint(20) NOT NULL DEFAULT '0' COMMENT '执行操作玩家俱乐部Id',"
                ...
                + "`type` int(2) NOT NULL DEFAULT '0' COMMENT '类型（0联盟1俱乐部）',"

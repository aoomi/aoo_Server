...
public final class PlayerWallet extends Feature {
    ...
    public boolean checkAndClubConsumeRoom(int count, GameType gameType, long clubId,
                                           Object status, long agentsId, int level) {
        return checkAndConsumeRoom(count, gameType);
    }
    ...
    public void backClubConsumeRoom(int count, GameType gameType, long clubId,
        ...
        if (count <= 0) return;
        backConsumeRoom(count, gameType);
    }
    ...
    private Mutation mutate(int requestedDelta, int gameId, ItemFlow reason,
        ...
        String lockKey = "playerWallet:" + accountID;
        try {
    ...
    private synchronized PlayerWalletBO walletRecord() {
        ...
        long accountID = accountID();
        String migrationLock = "playerWalletMigration:" + accountID;
        try {
            ...
            PlayerWalletBO current = service.findByAccountID(accountID);
            if (current == null && Config.isShare()) current = SharePlayerWalletMgr.getInstance().get(accountID);
            if (current == null) {
                current = new PlayerWalletBO(accountID);
    ...
    private long accountID() {
        ...
        if (accountID <= 0) throw new IllegalStateException("room-card wallet requires a persisted accountId");
        return accountID;
    }

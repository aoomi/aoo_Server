            ...
            this.getPosList().forEach(k -> {
                if (k.getPid() > 0 && !k.isPlayTheGame()) {
                    if (!business.global.billing.RoomBillingGuard.beginRefund(
                            this.getRoom().getRoomID(), k.getPlayer().getAccountID())) {
                        return;
                    }
                    k.getPlayer().getFeature(PlayerWallet.class).backConsumeRoom(
                            k.getPlayer().getRoomInfo().getConsumeCard(),
                            this.getRoom().getBaseRoomConfigure().getGameType());
                }
            });
        }
    }

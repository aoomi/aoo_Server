...
public final class RoomBillingGuard {
    ...
    public static boolean beginRefund(long roomId, long accountId) {
        if (roomId <= 0 || accountId <= 0) {
            throw new IllegalArgumentException("refund requires roomId and accountId");
        }
        RedisUtil.AtomicResult result = RedisUtil.setNxExResult(
                PREFIX + roomId + ":account:" + accountId + ":refund",
                "PROCESSING",
                RETENTION_SECONDS);
    ...
    public static void recordConsume(long roomId, long accountId, int amount) {
        if (roomId <= 0 || accountId <= 0 || amount < 0) {
            throw new IllegalArgumentException("invalid consume record");
        }
        RedisUtil.setex(
                PREFIX + roomId + ":account:" + accountId + ":consume",
                RETENTION_SECONDS,
                "amount=" + amount);
    }
}

...
public class UnionListMgr {
    ...
    public SData_Result onUnionSetConfig(CUnion_SetConfig setConfig, Player player) {
        ...
        if (execClubMember.isUnionMgr() || UnionDefine.UNION_POST_TYPE.UNION_CREATE.value() == unionMember.getUnionMemberBO().getType()) {
            ...
            if (!UnionDefine.UNION_CREATE_LIMIT.checkLimit(UnionDefine.UNION_CREATE_LIMIT.UNION_TOTAL_SCORE_LIMIT, setConfig.getUnionTotalScore())) {
                // 联盟总分错误
                return SData_Result.make(ErrorCode.UNION_TOTAL_SCORE_ERROR, "UNION_TOTAL_SCORE_ERROR");
            }
            ...
            if (setConfig.getOutSports() > setConfig.getUnionTotalScore() && UnionDefine.UNION_TYPE.NORMAL.value() == union.getUnionBO().getUnionType()) {
                // 淘汰分不能大于 联盟总分
...
//                union.checkDiamondsAttention();
            ...
            // 旧联盟总分
            long oldUnionTotalScore = union.getUnionBO().getUnionTotalScore();
            if (oldUnionTotalScore != setConfig.getUnionTotalScore()) {
                // 更新修改联盟总分
                this.updateUnionTotalScore(player.getPid(), setConfig.getUnionId(), setConfig.getClubId(), oldUnionTotalScore - setConfig.getUnionTotalScore());
            }
            ...
            union.setUnionSetConfigInfo(unionName, setConfig.getJoin(), setConfig.getQuit(), setConfig.getTableNum(), setConfig.getExpression(), setConfig.getState(), setConfig.getSports(), setConfig.getUnionTotalScore(), setConfig.getMatchRate(), outSports, setConfig.getPrizeType(), setConfig.getRanking(), setConfig.getValue(), player.getPid(), setConfig.getJoinClubSameUnion());
            ...
            union.setUnionSetConfigInfo(null, setConfig.getJoin(), setConfig.getQuit(), setConfig.getTableNum(), setConfig.getExpression(), -1, setConfig.getSports(), union.getUnionBO().getUnionTotalScore(), union.getUnionBO().getMatchRateValue(), union.getUnionBO().getOutSports(), union.getUnionBO().getPrizeType(), union.getUnionBO().getRanking(), union.getUnionBO().getValue(), player.getPid(), setConfig.getJoinClubSameUnion());
        }
        //修改共享联盟
        if (Config.isShare()) {
     ...
     * 更新修改联盟总分
     */
    private void updateUnionTotalScore(long pid, long unionId, long clubId, long value) {
        ClubMember clubMember = ClubMgr.getInstance().getClubMemberMgr().find(pid, clubId, Club_define.Club_Player_Status.PLAYER_JIARU);
        if (Objects.nonNull(clubMember)) {
            if (value > 0D) {
                clubMember.getClubMemberBO().execSportsPointInit(unionId, -value, ItemFlow.UNION_TOTAL_SCORE);
            } else {
                clubMember.getClubMemberBO().execSportsPointInit(unionId, Math.abs(value), ItemFlow.UNION_TOTAL_SCORE);
            }
        } else {
            CommLog.error("updateUnionTotalScore Pid:{},unionId:{},clubId:{},value:{}", pid, unionId, clubId, value);
        }
    }
    ...
    public SData_Result onUnionCreateTest(CUnion_Create data, Club club, Player player) {
        ...
        if (data.getUnionTotalScore() < 0L) {
            return SData_Result.make(ErrorCode.UNION_TOTAL_SCORE_ERROR, "UNION_CREATE_TOTAL_SCORE_ERROR");
        }
        if (data.getOutSports() > data.getUnionTotalScore()) {
        ...
        if (!UnionDefine.UNION_CREATE_LIMIT.checkLimit(UnionDefine.UNION_CREATE_LIMIT.UNION_TOTAL_SCORE_LIMIT, data.getUnionTotalScore())) {
            // 联盟总分错误
            return SData_Result.make(ErrorCode.UNION_TOTAL_SCORE_ERROR, "UNION_TOTAL_SCORE_ERROR");
        }
        ...
        // 联盟总分
        unionBO.setUnionTotalScore(data.getUnionTotalScore());
        ////钻石提醒 全员默认100
        ...
        unionBO.setCaseStatus(1);
        unionBO.setShareStatus(1);
        ...
        if (unionId <= 0L) {
            ...
            return SData_Result.make(ErrorCode.ErrorSysMsg, "创建联盟失败");
        }
        ...
        clubMember.getClubMemberBO().execSportsPointInit(unionBO.getId(), data.getUnionTotalScore(), ItemFlow.UNION_TOTAL_SCORE);
    ...
    public SData_Result changeUnionType(long unionId, int type) {
        ...
        if (UnionDefine.UNION_TYPE.ZhongZhi.equals(UnionDefine.UNION_TYPE.valueOf(type))) {
            ...
            // 旧联盟总分
            long oldUnionTotalScore = union.getUnionBO().getUnionTotalScore();
            if (oldUnionTotalScore != 0) {
                // 更新修改联盟总分
                this.updateUnionTotalScore(union.getUnionBO().getOwnerId(), unionId, union.getUnionBO().getClubId(), oldUnionTotalScore - 0);
            }

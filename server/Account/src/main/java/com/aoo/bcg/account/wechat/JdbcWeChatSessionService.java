...
import com.aoo.bcg.account.SessionReplacementNotifier;
...
public final class JdbcWeChatSessionService implements WeChatHttpRoutes.LoginHandler,
    ...
    private final DataSource source; private final WeChatOAuthClient oauth; private final Clock clock; private final SessionReplacementNotifier replacements; private final SecureRandom random=new SecureRandom();
    public JdbcWeChatSessionService(DataSource source,WeChatOAuthClient oauth,Clock clock){this(source,oauth,clock,SessionReplacementNotifier.none());}
    public JdbcWeChatSessionService(DataSource source,WeChatOAuthClient oauth,Clock clock,SessionReplacementNotifier replacements){this.source=Objects.requireNonNull(source);this.oauth=Objects.requireNonNull(oauth);this.clock=Objects.requireNonNull(clock);this.replacements=Objects.requireNonNull(replacements);}
    ...
    @Override public Object login(String code,String device,String channel,String version,String ip,String mode){
        ...
        LoginResult issued=tx(c->{long account=account(c,ticket);long generation=allowedGeneration(c,account,true);Instant now=clock.instant();
            ...
            return new LoginResult(account,session,generation,new TokenPair(access,refresh,ae,re));
        });
        if(single)replacements.replaced(issued.accountId(),issued.sessionId(),issued.authGeneration());
    ...
    private long account(Connection c,WeChatOAuthClient.Ticket t)throws SQLException{String sql="SELECT account_id FROM aoo_account_identity WHERE status='ACTIVE' AND verified=TRUE AND ((identity_type='WECHAT_OPEN' AND value_hash=?) OR (identity_type='WECHAT_UNION' AND value_hash=?)) FOR UPDATE";try(var p=c.prepareStatement(sql)){p.setString(1,hash(oauth.appId()+":"+t.openId()));p.setString(2,hash(t.unionId()==null?"":t.unionId()));try(var r=p.executeQuery()){Set<Long> accounts=new HashSet<>();while(r.next())accounts.add(r.getLong(1));if(accounts.isEmpty())throw new SecurityException("WeChat identity is not bound");if(accounts.size()!=1)throw new IllegalStateException("ambiguous WeChat identity conflict");return accounts.iterator().next();}}}
    ...
    private record LoginResult(long accountId,String sessionId,long authGeneration,TokenPair tokens){}

...
class ZgcpGameProviderTest {
    @Test void creationEntryReadyStartOperationRotationReconnectSettlementAndReplay() {
        ZgcpGameProvider provider=new ZgcpGameProvider();
        GameRoomHandle room=provider.roomFactory().create(new RoomCreationContext(9210,10,Map.of("piao",1)));
        ZgcpAuthoritativeSession session=(ZgcpAuthoritativeSession)room.requireAuthoritativeSession();
        session.execute(command(1,1,"20",Map.of("action","join")));
        session.execute(command(2,1,"20",Map.of("action","CZGCPOpPiao","payload",Map.of("piaoHua",1))));
        ...
        session.execute(command(6,1,"20",Map.of("action","operation","payload",Map.of("opType",1,"huPoints",14))));
        assertEquals("FINISHED",session.viewFor(20).get("phase"));
        assertEquals(9210,session.settlement(1,ZgcpGameProvider.VERSION).roomId());
        assertEquals(0L,session.settlement(1,ZgcpGameProvider.VERSION).scoreDelta().values().stream().mapToLong(Long::longValue).sum());
    ...
    @Test void serviceProviderRunsAuthenticatedAuthoritativeLifecycle() {
        GameProvider provider = new ZgcpGameProvider();
        GameRoomHandle room = provider.roomFactory().create(new RoomCreationContext(9210, 10, Map.of()));
        ...
        handler.handle(room, command(2, 1, "20", Map.of("action", "CZGCPReadyRoom", "payload", Map.of())));
        GameCommandResult started = handler.handle(room, command(3, 0, "10", Map.of("action", "CZGCPStartGame", "payload", Map.of())));
    ...
    @Test void rejectsSeatSpoofingReplayAndWrongMessageFamily() {
        ZgcpGameProvider provider = new ZgcpGameProvider();
        GameRoomHandle room = provider.roomFactory().create(new RoomCreationContext(9212, 10, Map.of()));
        GameCommandHandler handler = provider.commandHandler().orElseThrow();
        handler.handle(room, command(1, 1, "20", Map.of("action", "join", "payload", Map.of())));
        ...
        RuleResult wrong = provider.ruleComponents().getFirst().execute(new GameCommandRequest("poker.dispatch", "x", 2, 9212, 0, "1.0.0", "20", 1, Map.of()));
        assertFalse(wrong.accepted());
    }

    @Test void persistedSnapshotRestoresReconnectViewWithoutHiddenStateLeak() {
        ZgcpGameProvider provider=new ZgcpGameProvider();
        GameRoomHandle room=provider.roomFactory().create(new RoomCreationContext(9210,10,Map.of("rounds",8)));
        GameCommandHandler handler=provider.commandHandler().orElseThrow();
    ...
    @Test void orderedAuthorityEventsReplayFromDurableSnapshotAndRejectGaps() {
        ZgcpGameProvider provider=new ZgcpGameProvider();
        ZgcpAuthoritativeSession session=(ZgcpAuthoritativeSession)provider.roomFactory().create(new RoomCreationContext(9210,10,Map.of())).requireAuthoritativeSession();
    ...
    private static GameCommandRequest command(long sequence,int seat,String player,Map<String,Object> body) {
        return new GameCommandRequest("longcard.zgcp.dispatch", "request-"+sequence+"-"+player, sequence, 9210, 0, "1.0.0", player, seat, body);
    }
}

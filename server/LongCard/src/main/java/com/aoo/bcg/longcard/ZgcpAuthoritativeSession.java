...
/** Sole mutable ZGCP authority; client payloads are hints and never state. */
public final class ZgcpAuthoritativeSession implements AuthoritativeGameSession {
    ...
    private final Map<Integer,Integer> piao = new LinkedHashMap<>();
    ...
    public ZgcpAuthoritativeSession(long roomId,long ownerId,RulePayload rules) {
        if(roomId<=0||ownerId<=0) throw new IllegalArgumentException("invalid ZGCP room");
        this.roomId=roomId;this.ownerId=ownerId;this.rules=rules==null?RulePayload.empty():rules;seats.put(0,ownerId);
    }
    static ZgcpAuthoritativeSession restore(Map<String,Object> snapshot) {
        if(number(snapshot.get("schemaVersion"),-1)!=1)throw new IllegalArgumentException("unsupported ZGCP snapshot schema");
        long room=longNumber(snapshot.get("roomId")); long owner=longNumber(snapshot.get("ownerId"));
        ZgcpAuthoritativeSession session=new ZgcpAuthoritativeSession(room,owner,RulePayload.copyOf(stringMap(snapshot.get("rules"))));
        ...
        Object encodedPiao=snapshot.get("piao");if(encodedPiao instanceof Map<?,?>)stringMap(encodedPiao).forEach((seat,value)->session.piao.put(Integer.parseInt(seat),number(value,0)));
        ...
        if(!session.invariantViolations().isEmpty())throw new IllegalArgumentException("invalid ZGCP snapshot: "+session.invariantViolations());
        return session;
    }
    @Override public synchronized GameCommandResult execute(GameCommandRequest request) {
        ...
        switch(action) {
            case "CZGCPGetRoomInfo", "get_room_info", "state" -> requireParticipant(player);
            case "join" -> join(request.seatId(),player);
            case "CZGCPReadyRoom", "ready" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");ready.add(request.seatId()); }
            case "CZGCPUnReadyRoom", "unready" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");ready.remove(request.seatId()); }
            case "CZGCPOpPiao", "CZGCPPiaoHua", "piao" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");int value=number(payload(request.body()).asMap().get("piaoHua"),0);if(value<0||value>1)throw new IllegalArgumentException("invalid ZGCP piao");piao.put(request.seatId(),value); }
            case "CZGCPStartGame", "start" -> start(player);
            case "CZGCPOpCard", "operation" -> operate(request,player);
            default -> throw new IllegalArgumentException("unsupported ZGCP action: "+action);
        }
        lastSequence=request.sequence();if(!action.equals("state")&&!action.equals("get_room_info")&&!action.equals("CZGCPGetRoomInfo")){stateVersion=Math.addExact(stateVersion,1);events.add(Map.of("version",stateVersion,"after",snapshot()));}
        return new GameCommandResult("longcard.zgcp.response",request.requestId(),viewFor(player));
    }
    ...
    private void operate(GameCommandRequest request,long player){own(request.seatId(),player);if(state==null)throw new IllegalStateException("round not started");CommandPayload payload=payload(request.body());int op=number(payload.asMap().get("opType"),-1);Object raw=payload.asMap().get("cardID");if(raw==null)raw=payload.asMap().get("cardId");List<Integer>cards=raw instanceof Collection<?> c?c.stream().map(v->number(v,-1)).toList():raw==null?List.of():List.of(number(raw,-1));LongCardOperation operation=switch(op){case 7->LongCardOperation.DISCARD;case 6->LongCardOperation.CHI;case 2->LongCardOperation.PENG;case 1,9,75->LongCardOperation.HU;case 8->LongCardOperation.PASS;case 96->LongCardOperation.STEAL;case 99,124,125->LongCardOperation.CALL;default->throw new IllegalArgumentException("unsupported ZGCP operation: "+op);};state=BuiltinLongCardFamilies.require("ZGCP").engine().apply(state,new LongCardCommand(operation,request.seatId(),cards),new LongCardRuleContext(number(payload.asMap().get("huPoints"),0)));}
    @Override public synchronized Map<String,Object> viewFor(long viewer){Integer own=seat(viewer);Map<Integer,Object>visible=new LinkedHashMap<>();seats.forEach((seat,id)->{List<Integer>cards=state==null?List.of():state.hands().get(seat);visible.put(seat,Map.of("playerId",id,"ready",ready.contains(seat),"piao",piao.getOrDefault(seat,-1),"cards",Objects.equals(own,seat)||state!=null&&state.phase()==LongCardPhase.FINISHED?cards:Collections.nCopies(cards.size(),0),"cardCount",cards.size()));});Map<String,Object>out=new LinkedHashMap<>();out.put("roomID",roomId);out.put("roomId",roomId);out.put("gameCode","zgcp");out.put("started",state!=null);out.put("currentSeat",state==null?-1:state.currentSeat());out.put("phase",state==null?"WAITING":state.phase().name());out.put("winnerSeat",state==null||state.winnerSeat()==null?-1:state.winnerSeat());out.put("seats",Map.copyOf(visible));out.put("stateVersion",stateVersion);return Map.copyOf(out);}
    ...
    private Map<String,Object> snapshot(){Map<String,Object>out=new LinkedHashMap<>();out.put("schemaVersion",1);out.put("roomId",roomId);out.put("ownerId",ownerId);out.put("players",Map.copyOf(seats));out.put("ready",List.copyOf(ready));out.put("piao",Map.copyOf(piao));out.put("state",state==null?"WAITING":encodeState(state));out.put("rules",rules.asMap());out.put("lastSequence",lastSequence);out.put("stateVersion",stateVersion);return Map.copyOf(out);}
    static StatePayload replay(StatePayload base,List<Object> orderedEvents){Map<String,Object> current=new LinkedHashMap<>(base.asMap());long version=longNumber(current.getOrDefault("stateVersion",0));for(Object raw:orderedEvents){Map<String,Object>event=stringMap(raw);long next=longNumber(event.get("version"));if(next!=version+1)throw new IllegalArgumentException("ZGCP event version gap");current=new LinkedHashMap<>(stringMap(event.get("after")));if(longNumber(current.get("stateVersion"))!=next)throw new IllegalArgumentException("ZGCP event payload version mismatch");version=next;}return StatePayload.copyOf(current);}
    ...
    @Override public synchronized SettlementPayload settlement(int round,String version){if(state==null||state.phase()!=LongCardPhase.FINISHED)throw new IllegalStateException("round not finished");if(!ZgcpGameProvider.VERSION.equals(version))throw new IllegalArgumentException("play version mismatch");int cap=number(rules.asMap().get("fanshushangxian"),3);int add=number(rules.asMap().get("chaofanjiadi"),0);long unit=(1L<<Math.max(0,Math.min(cap,10)))+Math.max(0,add);int win=state.winnerSeat();Map<Long,Long>delta=new LinkedHashMap<>();seats.forEach((seat,id)->delta.put(id,seat==win?unit*(seats.size()-1):-unit));return new SettlementPayload(roomId,round,version,delta);}
    ...
    private static long longNumber(Object value){if(value==null)throw new IllegalArgumentException("missing ZGCP snapshot number");return value instanceof Number n?n.longValue():Long.parseLong(String.valueOf(value));}
    private static Collection<?> collection(Object value){if(!(value instanceof Collection<?> c))throw new IllegalArgumentException("invalid ZGCP snapshot collection");return c;}
    @SuppressWarnings("unchecked") private static Map<String,Object> stringMap(Object value){if(!(value instanceof Map<?,?> map))throw new IllegalArgumentException("invalid ZGCP snapshot map");Map<String,Object>out=new LinkedHashMap<>();map.forEach((key,item)->out.put(String.valueOf(key),item));return out;}
}

...
/** Sole mutable AYCP authority; client payloads are hints and never state. */
public final class AycpAuthoritativeSession implements AuthoritativeGameSession {
    ...
    public AycpAuthoritativeSession(long roomId,long ownerId,RulePayload rules) {
        if(roomId<=0||ownerId<=0) throw new IllegalArgumentException("invalid AYCP room");
        this.roomId=roomId;this.ownerId=ownerId;this.rules=rules==null?RulePayload.empty():rules;seats.put(0,ownerId);
    }
    static AycpAuthoritativeSession restore(Map<String,Object> snapshot) {
        if(number(snapshot.get("schemaVersion"),-1)!=1)throw new IllegalArgumentException("unsupported AYCP snapshot schema");
        long room=longNumber(snapshot.get("roomId")); long owner=longNumber(snapshot.get("ownerId"));
        AycpAuthoritativeSession session=new AycpAuthoritativeSession(room,owner,RulePayload.copyOf(stringMap(snapshot.get("rules"))));
        ...
        if(!session.invariantViolations().isEmpty())throw new IllegalArgumentException("invalid AYCP snapshot: "+session.invariantViolations());
        return session;
    }
    @Override public synchronized GameCommandResult execute(GameCommandRequest request) {
        ...
        switch(action) {
            case "CAYCPGetRoomInfo", "get_room_info", "state" -> requireParticipant(player);
            case "join" -> join(request.seatId(),player);
            case "CAYCPReadyRoom", "ready" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");ready.add(request.seatId()); }
            case "CAYCPUnReadyRoom", "unready" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");ready.remove(request.seatId()); }
            case "CAYCPOpPiao", "CAYCPPiaoHua", "piao" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");int value=number(payload(request.body()).asMap().get("piaoHua"),0);if(value<0||value>1)throw new IllegalArgumentException("invalid AYCP piao");piao.put(request.seatId(),value); }
            case "CAYCPStartGame", "start" -> start(player);
            case "CAYCPOpCard", "operation" -> operate(request,player);
            default -> throw new IllegalArgumentException("unsupported AYCP action: "+action);
        }
        lastSequence=request.sequence();if(!action.equals("state")&&!action.equals("get_room_info")&&!action.equals("CAYCPGetRoomInfo")){stateVersion=Math.addExact(stateVersion,1);events.add(Map.of("version",stateVersion,"after",snapshot()));}
        return new GameCommandResult("longcard.aycp.response",request.requestId(),viewFor(player));
    }
    ...
    static StatePayload replay(StatePayload base,List<Object> orderedEvents){Map<String,Object> current=new LinkedHashMap<>(base.asMap());long version=longNumber(current.getOrDefault("stateVersion",0));for(Object raw:orderedEvents){Map<String,Object>event=stringMap(raw);long next=longNumber(event.get("version"));if(next!=version+1)throw new IllegalArgumentException("AYCP event version gap");current=new LinkedHashMap<>(stringMap(event.get("after")));if(longNumber(current.get("stateVersion"))!=next)throw new IllegalArgumentException("AYCP event payload version mismatch");version=next;}return StatePayload.copyOf(current);}
    ...
    @Override public synchronized SettlementPayload settlement(int round,String version){if(state==null||state.phase()!=LongCardPhase.FINISHED)throw new IllegalStateException("round not finished");if(!AycpGameProvider.VERSION.equals(version))throw new IllegalArgumentException("play version mismatch");Map<Long,Long>delta=new LinkedHashMap<>();seats.values().forEach(id->delta.put(id,0L));return new SettlementPayload(roomId,round,version,delta);}
    ...
    private static long longNumber(Object value){if(value==null)throw new IllegalArgumentException("missing AYCP snapshot number");return value instanceof Number n?n.longValue():Long.parseLong(String.valueOf(value));}
    private static Collection<?> collection(Object value){if(!(value instanceof Collection<?> c))throw new IllegalArgumentException("invalid AYCP snapshot collection");return c;}
    @SuppressWarnings("unchecked") private static Map<String,Object> stringMap(Object value){if(!(value instanceof Map<?,?> map))throw new IllegalArgumentException("invalid AYCP snapshot map");Map<String,Object>out=new LinkedHashMap<>();map.forEach((key,item)->out.put(String.valueOf(key),item));return out;}
}

...
/** Sole mutable ZGDSS authority; client payloads are hints and never state. */
public final class ZgdssAuthoritativeSession implements AuthoritativeGameSession {
    ...
    public ZgdssAuthoritativeSession(long roomId,long ownerId,RulePayload rules) {
        if(roomId<=0||ownerId<=0) throw new IllegalArgumentException("invalid ZGDSS room");
        this.roomId=roomId;this.ownerId=ownerId;this.rules=rules==null?RulePayload.empty():rules;seats.put(0,ownerId);
    }
    static ZgdssAuthoritativeSession restore(Map<String,Object> snapshot) {
        if(number(snapshot.get("schemaVersion"),-1)!=1)throw new IllegalArgumentException("unsupported ZGDSS snapshot schema");
        long room=longNumber(snapshot.get("roomId")); long owner=longNumber(snapshot.get("ownerId"));
        ZgdssAuthoritativeSession session=new ZgdssAuthoritativeSession(room,owner,RulePayload.copyOf(stringMap(snapshot.get("rules"))));
        ...
        if(!session.invariantViolations().isEmpty())throw new IllegalArgumentException("invalid ZGDSS snapshot: "+session.invariantViolations());
        return session;
    }
    @Override public synchronized GameCommandResult execute(GameCommandRequest request) {
        ...
        switch(action) {
            case "CZGDSSGetRoomInfo", "get_room_info", "state" -> requireParticipant(player);
            case "join" -> join(request.seatId(),player);
            case "CZGDSSReadyRoom", "ready" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");ready.add(request.seatId()); }
            case "CZGDSSUnReadyRoom", "unready" -> { own(request.seatId(),player); if(state!=null)throw new IllegalStateException("round started");ready.remove(request.seatId()); }
            case "CZGDSSStartGame", "start" -> start(player);
            case "CZGDSSOpCard", "operation" -> operate(request,player);
            default -> throw new IllegalArgumentException("unsupported ZGDSS action: "+action);
        }
        lastSequence=request.sequence();if(!action.equals("state")&&!action.equals("get_room_info")&&!action.equals("CZGDSSGetRoomInfo")){stateVersion=Math.addExact(stateVersion,1);events.add(Map.of("version",stateVersion,"after",snapshot()));}
        return new GameCommandResult("longcard.zgdss.response",request.requestId(),viewFor(player));
    }
    ...
    static StatePayload replay(StatePayload base,List<Object> orderedEvents){Map<String,Object> current=new LinkedHashMap<>(base.asMap());long version=longNumber(current.getOrDefault("stateVersion",0));for(Object raw:orderedEvents){Map<String,Object>event=stringMap(raw);long next=longNumber(event.get("version"));if(next!=version+1)throw new IllegalArgumentException("ZGDSS event version gap");current=new LinkedHashMap<>(stringMap(event.get("after")));if(longNumber(current.get("stateVersion"))!=next)throw new IllegalArgumentException("ZGDSS event payload version mismatch");version=next;}return StatePayload.copyOf(current);}
    ...
    @Override public synchronized SettlementPayload settlement(int round,String version){if(state==null||state.phase()!=LongCardPhase.FINISHED)throw new IllegalStateException("round not finished");if(!ZgdssGameProvider.VERSION.equals(version))throw new IllegalArgumentException("play version mismatch");int cap=number(rules.asMap().get("fanshushangxian"),3);int add=number(rules.asMap().get("chaofanjiadi"),0);int wild=number(rules.asMap().get("laizishuliang"),0);long unit=(1L<<Math.max(0,Math.min(cap,10)))+Math.max(0,add)+Math.max(0,wild);int win=state.winnerSeat();Map<Long,Long>delta=new LinkedHashMap<>();seats.forEach((seat,id)->delta.put(id,seat==win?unit*(seats.size()-1):-unit));return new SettlementPayload(roomId,round,version,delta);}
    ...
    private static long longNumber(Object value){if(value==null)throw new IllegalArgumentException("missing ZGDSS snapshot number");return value instanceof Number n?n.longValue():Long.parseLong(String.valueOf(value));}
    private static Collection<?> collection(Object value){if(!(value instanceof Collection<?> c))throw new IllegalArgumentException("invalid ZGDSS snapshot collection");return c;}
    @SuppressWarnings("unchecked") private static Map<String,Object> stringMap(Object value){if(!(value instanceof Map<?,?> map))throw new IllegalArgumentException("invalid ZGDSS snapshot map");Map<String,Object>out=new LinkedHashMap<>();map.forEach((key,item)->out.put(String.valueOf(key),item));return out;}
}

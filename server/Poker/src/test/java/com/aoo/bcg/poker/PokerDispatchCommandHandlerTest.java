...
final class PokerDispatchCommandHandlerTest {
    ...
    @Test void canonicalEnvelopeExposesHintAndContinueOperations() {
        ...
        handler.handle(room, request("ready", 4, "20", 0, "njpdk.CNJPDKReadyRoom", Map.of()));
        handler.handle(room, request("start", 5, "20", 0, "njpdk.CNJPDKStartGame", Map.of()));
        ...
        var hint = handler.handle(room, request("hint", 6, player, seat, "hint", Map.of("nextPlayerCardCount", nextPlayerCardCount)));
        assertEquals("hint", hint.body().get("action"));
        assertThrows(IllegalStateException.class,
                () -> handler.handle(room, request("continue", 7, player, seat, "njpdk.CNJPDKContinueGame", Map.of())));
    }
    ...
    @Test void playerControlsOnlyOwnTrusteeshipAfterRoundStarts() {
        ...
        handler.handle(room, request("ready", 4, "20", 0, "njpdk.CNJPDKReadyRoom", Map.of()));
        handler.handle(room, request("start", 5, "20", 0, "njpdk.CNJPDKStartGame", Map.of()));
        handler.handle(room, request("trust", 6, "21", 1, "njpdk.CNJPDKTrusteeship", Map.of("trusteeship", true)));
        ...
        assertThrows(SecurityException.class, () -> handler.handle(room,
                request("spoof", 7, "20", 1, "njpdk.CNJPDKTrusteeship", Map.of("trusteeship", false))));
        handler.handle(room, request("cancel", 8, "21", 1, "njpdk.CNJPDKTrusteeship", Map.of("trusteeship", false)));
    ...
    private static GameCommandRequest request(String id, long sequence, String user, int seat,
        ...
        return new GameCommandRequest("poker.njpdk.dispatch", id, sequence, 9, 1, "pdk-v1", user, seat,

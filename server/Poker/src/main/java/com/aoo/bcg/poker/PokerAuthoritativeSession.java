...
public final class PokerAuthoritativeSession
    ...
    switch (op) {
      ...
      case "magic_expression" -> {
        ...
        if (!family.rules().config().advancedRules().governance().interactionEnabled())
          throw new SecurityException("interaction disabled by room rule");
        CommandPayload command = commandBody(r);
        int expressionId = num(command.getOrDefault("expressionId", 0));
        int targetSeatId = num(command.getOrDefault("targetSeatId", -1));
        if (expressionId < 1 || expressionId > 9999)
          throw new IllegalArgumentException("expressionId out of range");
        if (!players.containsKey(targetSeatId) || targetSeatId == r.seatId())
          throw new IllegalArgumentException("target seat does not exist");
        changed = false;
        body = Map.of(
            "messageId", r.requestId(),
            "requestId", r.requestId(),
      ...
      case "voice" -> {
        ...
        if (!family.rules().config().advancedRules().governance().interactionEnabled())
          throw new SecurityException("interaction disabled by room rule");
        CommandPayload command = commandBody(r);
        long assetId = lng(command.getOrDefault("assetId", 0));
        long durationMs = lng(command.getOrDefault("durationMs", 0));
        String mimeType = String.valueOf(command.getOrDefault("mimeType", "")).trim();
        if (assetId <= 0) throw new IllegalArgumentException("assetId required");
        if (durationMs <= 0 || durationMs > 10_000)
          throw new IllegalArgumentException("authoritative voice duration required");
        if (mimeType.isEmpty() || mimeType.length() > 80)
          throw new IllegalArgumentException("authoritative voice mime type required");
        changed = false;
        body = Map.of(
            "messageId", r.requestId(),
            "requestId", r.requestId(),
  ...
  private void own(long player, int seat) {
    ...
    ownRoomSeat(player, seat);
  }

  private void ownRoomSeat(long player, int seat) {
    if (!Long.valueOf(player).equals(players.get(seat)))
      throw new SecurityException("seat not owned");
  }

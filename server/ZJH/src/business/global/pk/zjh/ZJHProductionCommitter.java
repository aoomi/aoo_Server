...
/** Production durability boundary for every successful mutating ZJH command. */
public final class ZJHProductionCommitter implements GameCommandCommitter {
    ...
    public void commit(GameRoomHandle room, GameCommandRequest request, GameCommandResult result) {
        ...
        active.persistence.persist(room.requireLegacyRoom(ZJHTable.class), request.roundNo(), sequence,
        ...
        synchronized (initLock) {
            ...
            if (current == null) {
                ...
                current = new Runtime(nodeId, new JdbcRoomLeaseStore(dataSource, clock),
                        new ZJHPersistenceService(new JdbcRoomEventJournal(dataSource, mapper),
    ...
    private record Runtime(String nodeId, JdbcRoomLeaseStore leases, ZJHPersistenceService persistence) { }
}

...
public class ClassScanner {
     ...
     * 类型加载器
     */
    private static ClassLoader classLader = Thread.currentThread().getContextClassLoader();
    ...
    public static Set<Class<?>> getClasses(String pack, Predicate<Class<?>> filter) {
        ...
        try {
            dirs = classLader.getResources(packageDirName);
            // 循环迭代下去
    ...
    public static Class<?> forName(String name) throws ClassNotFoundException {
        return classLader.loadClass(name);
    }
    ...
    public static List<Class> getAllClassByInterface(Class c) {
        ...
        if (c.isInterface() || Modifier.isAbstract(c.getModifiers())) {
            ...
            Iterator var5 = allClass.iterator();

            while (var5.hasNext()) {
                Class<?> cs = (Class) var5.next();
                if (c.isAssignableFrom(cs) && !c.equals(cs)) {
    ...
    public static List<Class<?>> getAllClassByInterface(Class<?> c, String packageName) {
        ...
        if (c.isInterface() || Modifier.isAbstract(c.getModifiers())) {
            ...
            while(var5.hasNext()) {
                Class<?> cs = (Class)var5.next();
                if (c.isAssignableFrom(cs) && !Modifier.isAbstract(cs.getModifiers()) && !c.equals(cs)) {
                    returnClassList.add(cs);
                }
            }
        }

...
public class MongoDBHelper<T> {
    ...
    @Autowired
    @Qualifier(value = "mongoGameTemplate")
    ...
    public void insert(T info, String collectionName) {
        this.mongoTemplate.insert(info, collectionName);
    }


    /**
     * 功能描述: 往对应的集合中插入一条数据存在则更新
     *
     * @param info           存储对象
     * @param collectionName 集合名称
     * @return:void
    ...
    /**
     * Read a numeric field from a document in a named compatibility database.
     * This is used only for monotonic sequence migration; normal writes stay
     * on the configured MongoTemplate database.
     */
    public long readLongField(String databaseName, String collectionName, Object id, String fieldName) {
        Document document = mongoTemplate.getMongoDatabaseFactory()
    ...
    public String databaseName() {
        return mongoTemplate.getMongoDatabaseFactory().getMongoDatabase().getName();
    }
     ...
     * @return:void
     */
    public void updateByIdBean(String id, String collectionName, T info) {
        Query query = new Query(Criteria.where("id").is(id));
        Update update = new Update();
        BeanMap beanMap = BeanMap.create(info);
        for (Object key : beanMap.keySet()) {
            if (!"id".equals(key.toString())) {
                update.set(key.toString(), beanMap.get(key));
            }
        }
        mongoTemplate.updateMulti(query, update, info.getClass(), collectionName);
    }
    ...
    public boolean exists(Query query, Class<T> clazz, String collectionName) {
     ...
     * @return
     */
    public List<T> findAllAndRemove(String collectName,Query query,Class<T> clazz){
        List<T> result= mongoTemplate.findAllAndRemove(query,clazz,collectName);
        return  result;
    }
     ...
     * @return
     */
    public long count(String collectName,Query query,Class<T> clazz){

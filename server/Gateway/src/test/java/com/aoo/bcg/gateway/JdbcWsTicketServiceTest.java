...
class JdbcWsTicketServiceTest {
    ...
    @Test void authenticatesIssuesAndAtomicallyRejectsReplay(){var service=new JdbcWsTicketService(source,clock);var issued=service.issue("access-secret","device-a","https://game.example","request-1");assertEquals(30,Duration.between(clock.instant(),issued.expiresAt()).toSeconds());assertEquals(7,service.consume(issued.ticket(),"device-a","https://game.example").userId());assertEquals(GatewayErrorCode.WS_TICKET_REJECTED,assertThrows(GatewayTicketException.class,()->service.consume(issued.ticket(),"device-a","https://game.example")).code());}
    @Test void rejectsWrongDeviceAndOriginWithoutBurningValidTicket(){var service=new JdbcWsTicketService(source,clock);var issued=service.issue("access-secret","device-a","https://game.example","request-2");assertEquals(GatewayErrorCode.WS_TICKET_REJECTED,assertThrows(GatewayTicketException.class,()->service.consume(issued.ticket(),"device-b","https://game.example")).code());assertEquals(GatewayErrorCode.WS_TICKET_REJECTED,assertThrows(GatewayTicketException.class,()->service.consume(issued.ticket(),"device-a","https://evil.example")).code());assertEquals(7,service.consume(issued.ticket(),"device-a","https://game.example").userId());}
    @Test void rejectsExpiredTicketAndDuplicateIssueRequest(){var service=new JdbcWsTicketService(source,clock);var issued=service.issue("access-secret","device-a","https://game.example","request-3");assertEquals(GatewayErrorCode.IDEMPOTENCY_CONFLICT,assertThrows(GatewayTicketException.class,()->service.issue("access-secret","device-a","https://game.example","request-3")).code());clock.advance(Duration.ofSeconds(30));assertEquals(GatewayErrorCode.WS_TICKET_EXPIRED,assertThrows(GatewayTicketException.class,()->service.consume(issued.ticket(),"device-a","https://game.example")).code());}
    @Test void rejectsMismatchedBearerDevice(){var service=new JdbcWsTicketService(source,clock);assertEquals(GatewayErrorCode.UNAUTHORIZED,assertThrows(GatewayTicketException.class,()->service.issue("access-secret","device-b","https://game.example","request-4")).code());}
    @Test void browserUpgradeUsesTicketBoundDeviceAndRechecksLiveAuthority()throws Exception{
        ...
        var valid=service.issue("access-secret","device-a","https://game.example","browser-1");
        assertEquals("device-a",service.consume(valid.ticket(),"https://game.example").deviceFingerprint());
        var revoked=service.issue("access-secret","device-a","https://game.example","browser-2");
        ...
        assertEquals(GatewayErrorCode.WS_TICKET_REJECTED,assertThrows(GatewayTicketException.class,()->service.consume(revoked.ticket(),"https://game.example")).code());
    }

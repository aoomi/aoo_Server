...
final class GatewayWebSocketFrameHandlerTest {
    @Test void heartbeatDoesNotRequireRoomSession() throws Exception {
        ...
        var handler=new GatewayWebSocketFrameHandler(new ConnectionIdentity(7,"d","https://game.example"),router,(identity,frame)->{resolutions.incrementAndGet();throw new AssertionError("session resolution not expected");},GatewayWebSocketFrameHandler.BroadcastSink.none(),json,clock);
    ...
    @Test void accountSessionUsesNonRoomDispatcherWithoutResolvingGameRoom() throws Exception {
        ...
        var handler=new GatewayWebSocketFrameHandler(new ConnectionIdentity(7,"d","https://game.example"),router,(identity,frame)->{resolutions.incrementAndGet();throw new AssertionError("session resolution not expected");},GatewayWebSocketFrameHandler.BroadcastSink.none(),(identity,frame)->Map.of("pid",identity.userId()),json,clock);
    ...
    @Test void lobbyAndClubMessagesNeverEnterRoomAuthorityWhileRoomMessagesFailClosed() throws Exception {
        ...
        var router=new GameWebSocketRouter(new GameRegistry(),id->{throw new AssertionError("room lookup not expected");},new WebSocketRequestGuard(clock,Duration.ofSeconds(30)),new InMemoryIdempotencyStore<>(clock),Duration.ofHours(24));
        var handler=new GatewayWebSocketFrameHandler(new ConnectionIdentity(7,"d","https://game.example"),router,(identity,frame)->{resolutions.incrementAndGet();throw new AssertionError("room session not expected");},GatewayWebSocketFrameHandler.BroadcastSink.none(),(identity,frame)->{nonRoomCalls.incrementAndGet();return Map.of("action",frame.body().get("action"));},json,clock);
        EmbeddedChannel channel=new EmbeddedChannel(handler);
    ...
    @Test void realTextFramesReachRouterCorrelateResponsesBroadcastOnceAndReplayIdempotently() throws Exception {
        ...
        ObjectMapper json=new ObjectMapper();ConnectionIdentity identity=new ConnectionIdentity(7,"device","https://game.example");
        ...
        channel.writeInbound(new TextWebSocketFrame(request));TextWebSocketFrame replay=channel.readOutbound();Map<?,?> replayEnvelope=json.readValue(replay.text(),Map.class);replay.release();
    ...
    @Test void forgedVersionAndSequenceViolationProduceMachineFailureAndClose() throws Exception {
        ...
        var handler=new GatewayWebSocketFrameHandler(new ConnectionIdentity(7,"d","https://game.example"),new GameWebSocketRouter(games,id->room,new WebSocketRequestGuard(clock,Duration.ofSeconds(30)),new InMemoryIdempotencyStore<>(clock),Duration.ofHours(24)),(id,frame)->new GatewayWebSocketFrameHandler.SessionBinding(id.userId(),new ConnectionSession("7","100",0,"v1",0)),GatewayWebSocketFrameHandler.BroadcastSink.none(),json,clock);
        EmbeddedChannel channel=new EmbeddedChannel(handler);
        ...
        channel.writeInbound(new TextWebSocketFrame(forged));assertInstanceOf(TextWebSocketFrame.class,channel.readOutbound());assertInstanceOf(CloseWebSocketFrame.class,channel.readOutbound());channel.finishAndReleaseAll();
    }

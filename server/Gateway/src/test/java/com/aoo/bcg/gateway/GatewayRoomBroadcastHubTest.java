...
final class GatewayRoomBroadcastHubTest {
    @Test void broadcastsQuickTextPayloadToEveryRoomConnection() throws Exception {
        ...
        EmbeddedChannel second = new EmbeddedChannel(DefaultChannelId.newInstance(), true, false,
                new ChannelInboundHandlerAdapter());
        EmbeddedChannel third = new EmbeddedChannel(DefaultChannelId.newInstance(), true, false,
                new ChannelInboundHandlerAdapter());
        ...
        hub.connected(third.pipeline().firstContext(), new ConnectionIdentity(33, "device-33", "https://game.example", "page-33"),
                new ConnectionSession("33", "9001", 2, "xqp-equivalent-1", 0));
        ...
        assertQuickText(json, third.readOutbound(), payload);
        first.finishAndReleaseAll();
        second.finishAndReleaseAll();
        third.finishAndReleaseAll();
    }

    @Test void broadcastsEverySocialPayloadToEveryRoomConnection() throws Exception {
        for (String action : List.of("magic_expression", "voice")) {
            ObjectMapper json = new ObjectMapper();
            Clock clock = Clock.fixed(Instant.parse("2026-09-10T12:00:00Z"), ZoneOffset.UTC);
            GatewayRoomBroadcastHub hub = new GatewayRoomBroadcastHub((roomId, playerId) ->
                    fail("social events must not be replaced by a state snapshot"), json, clock);
            EmbeddedChannel first = new EmbeddedChannel(DefaultChannelId.newInstance(), true, false,
                    new ChannelInboundHandlerAdapter());
            EmbeddedChannel second = new EmbeddedChannel(DefaultChannelId.newInstance(), true, false,
                    new ChannelInboundHandlerAdapter());
            ConnectionSession sender = new ConnectionSession("11", "9001", 0, "pdk-v1", 0);
            hub.connected(first.pipeline().firstContext(), new ConnectionIdentity(11, "device-11", "https://game.example", "page-11"), sender);
            hub.connected(second.pipeline().firstContext(), new ConnectionIdentity(22, "device-22", "https://game.example", "page-22"),
                    new ConnectionSession("22", "9001", 1, "pdk-v1", 0));
            WebSocketFrame request = new WebSocketFrame("2.0", "common.room.dispatch", "req", action + "-1", 4,
                    "trace-social", "9001", 1, "pdk-v1", clock.millis(), Map.of("action", action));
            Map<String,Object> payload = Map.of("requestId", action + "-1", "sourceSeatId", 0);

            hub.publish(sender, request, new GameCommandResult("common.room.dispatch_resp", action + "-1", payload));

            assertSocial(json, first.readOutbound(), "room." + action, payload);
            assertSocial(json, second.readOutbound(), "room." + action, payload);
            first.finishAndReleaseAll();
            second.finishAndReleaseAll();
        }
    }
    ...
    private static void assertQuickText(ObjectMapper json, TextWebSocketFrame frame,
        ...
        assertSocial(json, frame, "room.quick_text", payload);
    }

    private static void assertSocial(ObjectMapper json, TextWebSocketFrame frame, String event,
            Map<String,Object> payload) throws Exception {
        assertNotNull(frame);
        Map<?, ?> envelope = json.readValue(frame.text(), Map.class);
        frame.release();
        assertEquals(event, envelope.get("msgId"));

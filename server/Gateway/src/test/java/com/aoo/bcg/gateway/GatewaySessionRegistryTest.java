...
final class GatewaySessionRegistryTest {
    ...
    @Test void templateInvalidationReachesEveryAuthenticatedSocketWithScopeOnly() throws Exception {
        var registry=new GatewaySessionRegistry();var json=new ObjectMapper();
        var clock=Clock.fixed(Instant.parse("2026-09-13T00:00:00Z"),ZoneOffset.UTC);
        var first=new EmbeddedChannel();var second=new EmbeddedChannel();
        registry.connected(first,new ConnectionIdentity(41,"device-a","https://game.example","session-a",1,"page-a"),json,clock);
        registry.connected(second,new ConnectionIdentity(42,"device-b","https://game.example","session-b",1,"page-b"),json,clock);

        assertEquals(2,registry.publishClubRoomTemplatesChanged(722583,0,
                "club.CClubCreateGameSetChangge","request-1","trace-1",json,clock));
        for(EmbeddedChannel channel:java.util.List.of(first,second)){
            TextWebSocketFrame frame=channel.readOutbound();JsonNode envelope=json.readTree(frame.text());frame.release();
            assertEquals("club.room_templates_changed",envelope.path("msgId").asText());
            assertEquals(722583,envelope.path("body").path("payload").path("clubId").asLong());
            assertEquals("club.CClubCreateGameSetChangge",envelope.path("body").path("payload").path("action").asText());
            assertFalse(envelope.path("body").path("payload").has("templates"));
        }
        registry.disconnected(first);registry.disconnected(second);
        first.finishAndReleaseAll();second.finishAndReleaseAll();
    }

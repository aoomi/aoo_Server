...
public final class JdbcWsTicketService {
    ...
    public Issued issue(String accessToken,String device,String origin,String idempotencyKey) {
        require(accessToken,"access token");require(device,"device");require(origin,"origin");require(idempotencyKey,"idempotency key");
        ...
        return tx(c->{Principal p=authorize(c,accessToken,device);if(!risk.decide(RiskAdmissionAuthority.Action.LOGIN,p.accountId,device,null,idempotencyKey).allowed())throw new RiskAdmissionException();Instant now=clock.instant(), expiry=now.plus(TTL);String raw=token();
            ...
            return new Issued(raw,expiry,p.accountId,device,origin);
        });
    }

    public ConnectionIdentity consume(String raw,String device,String origin) {
        require(raw,"ticket");require(device,"device");require(origin,"origin");
        return tx(c->{Ticket t;
            try(PreparedStatement q=c.prepareStatement("SELECT account_id,device_id,allowed_origin,expires_at FROM gateway_ws_ticket WHERE ticket_hash=? FOR UPDATE")){
                q.setString(1,hash(raw));try(ResultSet r=q.executeQuery()){if(!r.next())throw new GatewayTicketException(GatewayErrorCode.WS_TICKET_REJECTED);t=new Ticket(r.getLong(1),r.getString(2),r.getString(3),r.getTimestamp(4).toInstant());}
            }
    ...
    private Principal authorize(Connection c,String raw,String device)throws SQLException{
        try(PreparedStatement q=c.prepareStatement("SELECT s.account_id,s.session_id,s.device_id,s.access_expires_at,s.auth_generation,a.auth_generation,a.banned_until,s.revoked_at,d.revoked_at FROM aoo_account_session s JOIN aoo_account a ON a.account_id=s.account_id JOIN aoo_account_device d ON d.account_id=s.account_id AND d.device_id=s.device_id WHERE s.access_hash=?")){
            q.setString(1,hash(raw));try(ResultSet r=q.executeQuery()){if(!r.next()||r.getTimestamp(8)!=null||r.getTimestamp(9)!=null||!device.equals(r.getString(3))||r.getLong(5)!=r.getLong(6)||!clock.instant().isBefore(r.getTimestamp(4).toInstant())||(r.getTimestamp(7)!=null&&clock.instant().isBefore(r.getTimestamp(7).toInstant())))throw new GatewayTicketException(GatewayErrorCode.UNAUTHORIZED);return new Principal(r.getLong(1),r.getString(2));}
        }
    }
    ...
    @FunctionalInterface private interface Sql<T>{T run(Connection c)throws Exception;} private record Principal(long accountId,String sessionId){} private record Ticket(long accountId,String device,String origin,Instant expiry){}
    public record Issued(String ticket,Instant expiresAt,long accountId,String deviceId,String origin){}
}

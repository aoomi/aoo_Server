...
public final class GatewayWebSocketFrameHandler extends SimpleChannelInboundHandler<io.netty.handler.codec.http.websocketx.WebSocketFrame> {
    ...
    @Override protected void channelRead0(ChannelHandlerContext ctx, io.netty.handler.codec.http.websocketx.WebSocketFrame wire) {
        ...
        try {
            ...
            if (WebSocketFrame.isNonRoomMessage(request.msgId())) {
                Object result=nonRoom.dispatch(identity, request);
                success(ctx, request, result, false);
                publishClubTemplateInvalidation(request);
                return;
            }
            ...
            if(request!=null&&WebSocketFrame.isNonRoomMessage(request.msgId()))
                failure(ctx, GatewayErrorCode.ROOM_ADMIN_FORBIDDEN, request, false, userMessage(failure));
            else failure(ctx, GatewayErrorCode.UNAUTHORIZED, request, true);
    ...
    private void publishClubTemplateInvalidation(WebSocketFrame request) {
        if(!"club.dispatch".equals(request.msgId()))return;
        String action=String.valueOf(request.body().getOrDefault("action",""));
        if(!java.util.Set.of("club.CClubCreateGameSet","club.CClubRoomCfgSave",
                "club.CClubCreateGameSetChangge","union.CUnionCreateRoom",
                "union.CUnionRoomCfgUpdate").contains(action))return;
        Object raw=request.body().get("payload");
        if(!(raw instanceof Map<?,?> payload))return;
        long clubId=number(payload.get("clubId")),unionId=number(payload.get("unionId"));
        GatewaySessionRegistry.global().publishClubRoomTemplatesChanged(clubId,unionId,action,
                request.requestId(),request.traceId(),json,clock);
    }

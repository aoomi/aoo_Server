...
public final class Cd299GameProvider implements GameProvider {
    private static final String DISPATCH="poker.CD299.dispatch";
    ...
    public Optional<GameCommandHandler>commandHandler(){var authoritative=new AuthoritativeSessionCommandHandler();return Optional.of((room,request)->authoritative.handle(room,unwrap(request)));}
    ...
    public List<RuleComponent<GameCommandRequest>>ruleComponents(){return List.of(new RuleComponent<>(){
        ...
        public RuleResult execute(GameCommandRequest request){
            ...
            String command=DISPATCH.equals(request.msgId())?String.valueOf(request.body().asMap().get("action")):request.msgId();
    ...
    private static GameCommandRequest unwrap(GameCommandRequest request){if(!DISPATCH.equals(request.msgId()))return request;Map<String,Object>body=request.body().asMap();String action=String.valueOf(body.get("action"));if(!COMMANDS.contains(action))throw new IllegalArgumentException("CD299_COMMAND_NOT_ALLOWED: [CD299] action="+action);Object rawPayload=body.get("payload");if(!(rawPayload instanceof Map<?,?>raw))throw new IllegalArgumentException("[CD299] dispatch payload must be an object");Map<String,Object>payload=new java.util.LinkedHashMap<>();raw.forEach((key,value)->payload.put(String.valueOf(key),value));return new GameCommandRequest(action,request.requestId(),request.sequence(),request.roomId(),request.roundNo(),request.playVersion(),request.authenticatedUserId(),request.seatId(),payload);}
}

...
public final class Cd299HandEvaluator {
    ...
    /** Canonical authority order used by every client: stronger pair first (screen-left), weaker pair second. Equal pairs keep the submitted first pair on the left. */
    public List<Integer> bigLeftOrder(List<Integer>ordered,boolean earthNineKing){SplitRank rank=split(ordered,earthNineKing);List<Integer>canonical=new ArrayList<>(4);canonical.addAll(rank.tail().cards());canonical.addAll(rank.head().cards());return List.copyOf(canonical);}

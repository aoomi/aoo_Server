...
public final class Cd299SettlementEngine {
    ...
    public record Result(Map<Long,Long> scoreDelta,long mangoPool,List<Integer> tailWinnerSeats){
        public Result{scoreDelta=Map.copyOf(scoreDelta);tailWinnerSeats=List.copyOf(tailWinnerSeats);if(scoreDelta.values().stream().mapToLong(Long::longValue).sum()!=0)throw new IllegalArgumentException("CD299 settlement must be zero-sum");}
    }
    public Result settle(List<Player> input,boolean headBigKeepsBase){
        ...
        if(active.isEmpty())return new Result(delta,players.stream().mapToLong(Player::mango).sum(),List.of());
        ...
        List<Player>tailWinners=active.size()==1?active:bestTailThenHead(active);long mangoPool=players.stream().mapToLong(Player::mango).sum();
        Map<Long,Long>mangoContributions=new LinkedHashMap<>();players.forEach(p->mangoContributions.put(p.playerId(),-p.mango()));
        transferShared(delta,tailWinners,mangoPool,mangoContributions);
        ...
        return new Result(delta,mangoPool,tailWinners.stream().map(Player::seat).toList());
    }

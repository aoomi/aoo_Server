...
class Cd299SettlementEngineTest {
 ...
 @Test void droppedPlayerPaysTailWinnerAndThreeFlowerRefundsBaseBet(){var rank=split(203,506,212,412);var r=engine.settle(List.of(p(0,10,1,3,6,false,false,rank),p(1,11,1,3,6,true,false,null),p(2,12,1,3,6,false,true,null)),false);assertEquals(0,r.scoreDelta().values().stream().mapToLong(Long::longValue).sum());assertTrue(r.scoreDelta().get(10L)>0);assertEquals(-3,r.scoreDelta().get(12L));}
 @Test void allThreeFlowerPlayersAreFullyRefunded(){var r=engine.settle(List.of(p(0,10,1,3,6,false,true,null),p(1,11,1,3,6,false,true,null)),false);assertEquals(0,r.scoreDelta().get(10L));assertEquals(0,r.scoreDelta().get(11L));}

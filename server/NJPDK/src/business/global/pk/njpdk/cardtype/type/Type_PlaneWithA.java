 ...
 * 飞机带一张
 */
public class Type_PlaneWithA extends NJPDKAbsType {

    private List<Integer> cardTypeList = new ArrayList<>(Arrays.asList(NJPDK_define.NJPDK_CARD_TYPE.PDK_CARD_TYPE_NOMARL.value(), NJPDK_define.NJPDK_CARD_TYPE.PDK_CARD_TYPE_FEIJI31.value()));

    @Override
    public boolean checkCard(ArrayList<Integer> cardList) {
        //飞机带一张
        TargetCardContainer cc = algContainer.getPlaneWithAByCardList(cardList, 0, maxCardValue, -1);
    ...
    public List<Integer> generateCardList(NJPDKALGParameter parameter) {
        ...
        if (cardTypeList.contains(previousOpType)) {
            ...
            if (previousOpType == NJPDK_define.NJPDK_CARD_TYPE.PDK_CARD_TYPE_FEIJI31.value()) {
                TargetCardContainer c1 = algContainer.getPlaneWithAByCardList(previousCardList, planeCompareValue, maxCardValue, planeLength);
                planeCompareValue = c1.planeCompareValue;
                planeLength = c1.planeLength;
            }
            TargetCardContainer cc = algContainer.getPlaneWithAByCardList(cardList, planeCompareValue, parameter.leftCardSize, planeLength);

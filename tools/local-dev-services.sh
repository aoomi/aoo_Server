...
start_gateway(){
  ...
  start_java gateway 8080 "$gateway_cp" com.aoo.bcg.gateway.GatewayApplication \
    ...
    GATEWAY_OFFICIAL_ORIGINS="$origins" GATEWAY_ALLOW_INSECURE_LAN_PREVIEW=true GATEWAY_VERSION_URL=http://127.0.0.1:8095 GATEWAY_ACCOUNT_URL=http://127.0.0.1:8096 GATEWAY_HALL_URL=http://127.0.0.1:8093 HALL_INTERNAL_URL=http://127.0.0.1:8093 HALL_INTERNAL_TOKEN="${AOO_LOCAL_HALL_INTERNAL_TOKEN:-local-hall-internal-token-at-least-32-bytes}" GATEWAY_SOCIAL_URL=http://127.0.0.1:8097 GATEWAY_MEDIA_URL=http://127.0.0.1:8102 AOO_MEDIA_INTERNAL_URL=http://127.0.0.1:8102 AOO_MEDIA_ROOM_TOKEN="${AOO_LOCAL_MEDIA_ROOM_TOKEN:-local-media-room-token-at-least-32-bytes}" GATEWAY_INTERNAL_TOKEN="${AOO_LOCAL_ROOM_AUTHORITY_TOKEN:-local-room-authority-token-at-least-32-bytes}" GATEWAY_INTERNAL_PORT=18080 GATEWAY_HTTP_PORT=8080
}

start_media(){
  local bootstrap_cp="$1"
  start_java media 8102 "$bootstrap_cp" com.aoo.bcg.bootstrap.BootstrapAPP \
    MEDIA_DATABASE_URL="$DB_URL" MEDIA_DATABASE_USER="$MYSQL_USER" MEDIA_DATABASE_PASSWORD="$MYSQL_PASSWORD" \
    MEDIA_S3_ENDPOINT=http://127.0.0.1:19000 MEDIA_S3_BUCKET=aoo-media MEDIA_S3_REGION=us-east-1 \
    MEDIA_S3_ACCESS_KEY="${AOO_LOCAL_MEDIA_S3_ACCESS_KEY:-replace-with-local-minio-user}" \
    MEDIA_S3_SECRET_KEY="${AOO_LOCAL_MEDIA_S3_SECRET_KEY:-replace-with-at-least-16-random-characters}" \
    MEDIA_CLEANUP_TOKEN="${AOO_LOCAL_MEDIA_CLEANUP_TOKEN:-local-media-cleanup-token-at-least-32-bytes}" \
    MEDIA_ROOM_TOKEN="${AOO_LOCAL_MEDIA_ROOM_TOKEN:-local-media-room-token-at-least-32-bytes}" MEDIA_HTTP_PORT=8102
}
